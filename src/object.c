...
void write_object(object_type type, FILE* src, repository* repo, object_id* out_oid)
{
    ...
    oid_hex hex = oid_tostring(out_oid);
    char prefix[3];
    strncpy(prefix, hex.hex, 2);
    prefix[2] = '\0';
...
{
    const char* hex_oid = hex->hex;
    char prefix[3];
    strncpy(prefix, hex_oid, 2);
    prefix[2] = '\0';
    const char* objfile_name = hex_oid + 2;
