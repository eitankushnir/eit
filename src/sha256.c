...
// BITWISE MACROS used in the hashing.
...
// Magical constants used during the hashing.
...
void sha256_init(SHA256_CTX *ctx) {
  // initialize the context struct with all the correct constants.
  ctx->datalen = 0;
  ctx->bitlen = 0;
...
char *oid_to_hex(struct object_id *oid, struct hex_oid *out) {
  ...
  for (int i = 0; i < 32; i++) {
    ...
    out->hex[i * 2 + 1] = hex_chars[oid->hash[i] & 0x0F];
  }
  out->hex[64] = '\0';
